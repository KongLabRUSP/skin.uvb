# |----------------------------------------------------------------------------------|
# | Project: Skin UVB SKH1 mouse model treated with UA/UAA                           |
...
# |          07/18/2018, DS: control, UVB and UVB+UA only                            |
# |----------------------------------------------------------------------------------|
...
dim(pct)
# 51410 out of 237,858 such rows. Remove.
...
# b. UAA vs. UVB at Week 2----
DMLtest.UAA.UVB.W2 <- DMLtest.multiFactor(DMLfit,
                                          coef = "trtUAA")
head(DMLtest.UAA.UVB.W2)
DMLtest.UAA.UVB.W2$chr <- as.numeric(as.character(DMLtest.UAA.UVB.W2$chr))

DMLtest.UAA.UVB.W2 <- data.table(merge(data.table(dt1[, gene:CpG],
                                                  pct[, c("X02w_UAA_0",
                                                          "X02w_UAA_1",
                                                          "X02w_UVB_0",
                                                          "X02w_UVB_1")]),
                                       ...
                                       by = c("chr",
                                              "pos")))
DMLtest.UAA.UVB.W2$mu.UAA2w <- (DMLtest.UAA.UVB.W2$X02w_UAA_0 + DMLtest.UAA.UVB.W2$X02w_UAA_1)/2
DMLtest.UAA.UVB.W2$mu.uvb2w <- (DMLtest.UAA.UVB.W2$X02w_UVB_0 + DMLtest.UAA.UVB.W2$X02w_UVB_1)/2

# Mean vs. difference----
DMLtest.UAA.UVB.W2$mu <- (DMLtest.UAA.UVB.W2$mu.UAA2w + DMLtest.UAA.UVB.W2$mu.uvb2w)/2
DMLtest.UAA.UVB.W2$diff <- (DMLtest.UAA.UVB.W2$mu.UAA2w - DMLtest.UAA.UVB.W2$mu.uvb2w)

# Green: hypomethylated in UVB; Red: hypermethylated in UVB
dtp1 <- DMLtest.UAA.UVB.W2[!is.na(stat), ]
tiff(filename = "tmp/skin_uvb_maplot_UAA_uvb_w2.tiff",
     height = 6,
     width = 6,
     units = 'in',
     res = 300,
...
plot(dtp1$diff ~ dtp1$mu,
     ...
     xlab = "Mean",
     ylab = "Difference",
     main = "Proportion of Methylation in UAA vs. UVB\nat Weeks 2, FDR < 0.1")
points(dtp1$diff[dtp1$fdrs < 0.1 & dtp1$diff > 0] ~ dtp1$mu[dtp1$fdrs < 0.1 & dtp1$diff > 0] ,
       pch = "x",
       col = "red")
points(dtp1$diff[dtp1$fdrs < 0.1 & dtp1$diff < 0] ~ dtp1$mu[dtp1$fdrs < 0.1 & dtp1$diff < 0] ,
...
uvb.hyper.2 <- unique(DMLtest.UAA.UVB.W2[fdrs < 0.1 & dtp1$diff < 0]$pos)
hyper <- uvb.hyper.1[uvb.hyper.1 %in% uvb.hyper.2]
l1 <- DMLtest.Ctrl.UVB.W2[pos %in% hyper, c("gene", 
                                            "pos", 
                                            "diff")]
l1$diff <- (-1)*l1$diff
l2 <- DMLtest.UAA.UVB.W2[pos %in% hyper, c("gene", 
                                           "pos", 
                                           "diff")]
# Venn diagram----
length(uvb.hyper.1)
...
uvb.hypo.2 <- unique(DMLtest.UAA.UVB.W2[fdrs < 0.1 & dtp1$diff > 0]$pos)
hypo <- uvb.hypo.1[uvb.hypo.1 %in% uvb.hypo.2]
l3 <- DMLtest.Ctrl.UVB.W2[pos %in% hypo, c("gene", 
                                           "pos", 
                                           "diff")]
l3$diff <- (-1)*l3$diff
l4 <- DMLtest.UAA.UVB.W2[pos %in% hypo, c("gene", 
                                          "pos", 
                                          "diff")]
# Venn diagram----
length(uvb.hypo.1)
...
# b. UAA vs. UVB at Week 15----
DMLtest.UAA.UVB.W15 <- DMLtest.multiFactor(DMLfit,
                                           coef = "trtUAA")
head(DMLtest.UAA.UVB.W15)
DMLtest.UAA.UVB.W15$chr <- as.numeric(as.character(DMLtest.UAA.UVB.W15$chr))

DMLtest.UAA.UVB.W15 <- data.table(merge(data.table(dt1[, gene:CpG],
                                                   pct[, c("X15w_UAA_0",
                                                           "X15w_UAA_1",
                                                           "X15w_UVB_0",
                                                           "X15w_UVB_1")]),
                                        ...
                                        by = c("chr",
                                               "pos")))
DMLtest.UAA.UVB.W15$mu.UAA15w <- (DMLtest.UAA.UVB.W15$X15w_UAA_0 + DMLtest.UAA.UVB.W15$X15w_UAA_1)/2
DMLtest.UAA.UVB.W15$mu.uvb15w <- (DMLtest.UAA.UVB.W15$X15w_UVB_0 + DMLtest.UAA.UVB.W15$X15w_UVB_1)/2

# Mean vs. difference----
DMLtest.UAA.UVB.W15$mu <- (DMLtest.UAA.UVB.W15$mu.UAA15w + DMLtest.UAA.UVB.W15$mu.uvb15w)/2
DMLtest.UAA.UVB.W15$diff <- (DMLtest.UAA.UVB.W15$mu.UAA15w - DMLtest.UAA.UVB.W15$mu.uvb15w)

# Green: hypomethylated in UVB; Red: hypermethylated in UVB
dtp1 <- DMLtest.UAA.UVB.W15[!is.na(stat), ]
tiff(filename = "tmp/skin_uvb_maplot_UAA_uvb_w15.tiff",
     height = 6,
     width = 6,
     units = 'in',
     res = 300,
...
uvb.hyper.2 <- unique(DMLtest.UAA.UVB.W15[fdrs < 0.1 & dtp1$diff < 0]$pos)
hyper <- uvb.hyper.1[uvb.hyper.1 %in% uvb.hyper.2]
l1 <- DMLtest.Ctrl.UVB.W15[pos %in% hyper, c("gene", 
                                             "pos", 
                                             "diff")]
l1$diff <- (-1)*l1$diff
l2 <- DMLtest.UAA.UVB.W15[pos %in% hyper, c("gene", 
                                            "pos", 
                                            "diff")]
# Venn diagram----
length(uvb.hyper.1)
...
uvb.hypo.2 <- unique(DMLtest.UAA.UVB.W15[fdrs < 0.1 & dtp1$diff > 0]$pos)
hypo <- uvb.hypo.1[uvb.hypo.1 %in% uvb.hypo.2]
l3 <- DMLtest.Ctrl.UVB.W15[pos %in% hypo, c("gene", 
                                            "pos", 
                                            "diff")]
l3$diff <- (-1)*l3$diff
l4 <- DMLtest.UAA.UVB.W15[pos %in% hypo, c("gene", 
                                           "pos", 
                                           "diff")]
# Venn diagram----
length(uvb.hypo.1)
...
# b. UAA vs. UVB at Week 25----
DMLtest.UAA.UVB.W25 <- DMLtest.multiFactor(DMLfit,
                                           coef = "trtUAA")
head(DMLtest.UAA.UVB.W25)
DMLtest.UAA.UVB.W25$chr <- as.numeric(as.character(DMLtest.UAA.UVB.W25$chr))

DMLtest.UAA.UVB.W25 <- data.table(merge(data.table(dt1[, gene:CpG],
                                                   pct[, c("X25w_UAA_0",
                                                           "X25w_UAA_1",
                                                           "X25w_UVB_0",
                                                           "X25w_UVB_1")]),
                                        ...
                                        by = c("chr",
                                               "pos")))
DMLtest.UAA.UVB.W25$mu.UAA25w <- (DMLtest.UAA.UVB.W25$X25w_UAA_0 + DMLtest.UAA.UVB.W25$X25w_UAA_1)/2
DMLtest.UAA.UVB.W25$mu.uvb25w <- (DMLtest.UAA.UVB.W25$X25w_UVB_0 + DMLtest.UAA.UVB.W25$X25w_UVB_1)/2

# Mean vs. difference----
DMLtest.UAA.UVB.W25$mu <- (DMLtest.UAA.UVB.W25$mu.UAA25w + DMLtest.UAA.UVB.W25$mu.uvb25w)/2
DMLtest.UAA.UVB.W25$diff <- (DMLtest.UAA.UVB.W25$mu.UAA25w - DMLtest.UAA.UVB.W25$mu.uvb25w)

# Green: hypomethylated in UVB; Red: hypermethylated in UVB
dtp1 <- DMLtest.UAA.UVB.W25[!is.na(stat), ]
tiff(filename = "tmp/skin_uvb_maplot_UAA_uvb_w25.tiff",
     height = 6,
     width = 6,
     units = 'in',
     res = 300,
...
uvb.hyper.2 <- unique(DMLtest.UAA.UVB.W25[fdrs < 0.1 & dtp1$diff < 0]$pos)
hyper <- uvb.hyper.1[uvb.hyper.1 %in% uvb.hyper.2]
l1 <- DMLtest.Ctrl.UVB.W25[pos %in% hyper, c("gene", 
                                             "pos", 
                                             "diff")]
l1$diff <- (-1)*l1$diff
l2 <- DMLtest.UAA.UVB.W25[pos %in% hyper, c("gene", 
                                            "pos", 
                                            "diff")]
# Venn diagram----
length(uvb.hyper.1)
...
uvb.hypo.2 <- unique(DMLtest.UAA.UVB.W25[fdrs < 0.1 & dtp1$diff > 0]$pos)
hypo <- uvb.hypo.1[uvb.hypo.1 %in% uvb.hypo.2]
l3 <- DMLtest.Ctrl.UVB.W25[pos %in% hypo, c("gene", 
                                            "pos", 
                                            "diff")]
l3$diff <- (-1)*l3$diff
l4 <- DMLtest.UAA.UVB.W25[pos %in% hypo, c("gene", 
                                           "pos", 
                                           "diff")]
# Venn diagram----
length(uvb.hypo.1)
